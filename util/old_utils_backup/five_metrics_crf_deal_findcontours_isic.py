...
#             print("img: {} iou: {.f} JA: {.f} AC: {.f} DI: {.f} SE: {.f} SP: {.f}".format(filename,iou,JA,AC,DI,SE,SP))
#             iou_sum += iou
#             JA_sum += JA
#             AC_sum += AC
#             DI_sum += DI
...
def five_single_folder(args,img_path, label_path):
        ...
        for filename in os.listdir(img_path):
            ...
            if pre is None or  target is None:
                ...
                continue
            iou,JA,AC,DI,SE,SP = iou_score(pre/255, target/255)
            print("img: {} iou: {:.4f} JA: {:.4f} AC: {:.4f} DI: {:.4f} SE: {:.4f} SP: {:.4f}".format(filename,iou,JA,AC,DI,SE,SP))
            iou_sum += iou
            JA_sum += JA
            AC_sum += AC
            DI_sum += DI
        ...
        with open(os.path.join(args.result_path,args.exp_idx,args.phase+"_"+args.epoch+"_crf/"+args.phase+"_results.txt"), "a+") as f:
            f.write("folder:%s\n" % (img_path))
            f.write("JA:%.4f \n" % (JA_sum/count))
            f.write("AC:%.4f \n" % (AC_sum/count))
            f.write("DI:%.4f \n" % (DI_sum/count))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--orig_img_path", default="/home/dw/tangsy/attention_gan_seam/AttentionGAN-master/datasets", type=str)
    ...
    parser.add_argument("--input_path", default="erode", type=str)
    ...
    list_result = []
    for t in t_list:
        t_crf_path = crf_path+"/"+str(t)+"_findcontours"
